  ...
  if(kart) {
    ...
    function ekraniCiz(data, ilkAcilisMi = false) {
        ...
        for (let tur in siraliGruplar) {
            ...
            let html = `<div class="sub-accordion-header" onclick="toggleSubAccordion('${globalSira}')"><div style="font-size: 14px; font-weight: 600; color: #e2e8f0; display:flex; align-items:center;"><svg id="sub-icon-${globalSira}" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right:8px; color:var(--text-muted); transition: transform 0.3s ease;"><polyline points="9 18 15 12 9 6"></polyline></svg>${modernTik}${tur}</div><div style="text-align:right;"><span style="font-size:12px; font-weight:800; color:${g.kalan === 0 ? 'var(--emerald)' : 'var(--amber)'}; letter-spacing: -0.2px;">₺${(g.odendi||0).toLocaleString('tr-TR')}</span><span style="font-size:11px; color:var(--text-muted); font-weight:600; margin-left:2px;"> / ₺${((g.odendi||0) + (g.kalan||0)).toLocaleString('tr-TR')}</span></div></div><div class="sub-accordion-content" id="sub-content-${globalSira}">`;
            g.liste.forEach(s => {
